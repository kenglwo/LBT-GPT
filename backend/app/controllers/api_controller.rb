    ...
    def save_student_data
        ...
        if data_id == 'answer_data' then
            ...
            if type == 'image_url' then
                ...
                # if image generated by DELL-3 is returned
                if data[:content].start_with?("https://") then
                    ...
                    # save the filepath of the image stored at locallhost into db
                    @latest_chatgpt_response = ConversationDatum.where(student_id: student_id, role: role).select(:id).last(1)
                    new_data = Dalle3ImageMappings.new(id_conversation_data: @latest_chatgpt_response[0].id, image_url_local: file_path)
                    api_status = new_data.save ? "success" : "failed"
                end
            ...
            if type == 'image_url' then
                ...
                if data[:content].start_with?("https://") then
                    ...
                    # save the filepath of the image stored at locallhost into db
                    @latest_chatgpt_response = QuizConversationDatum.where(student_id: student_id, role: role).select(:id).last(1)
                    new_data = Dalle3ImageMappingsQuiz.new(id_quiz_conversation_data: @latest_chatgpt_response[0].id, image_url_local: file_path)
                    api_status = new_data.save ? "success" : "failed"
                end
            end
        end
