...
const TAS_MISCONCEPTION_INST = `You are a friendly and encouraging professor who supervises interactions between a new teacher and an 18-year old student.
...
const TAS_HELP_INST = `You are a friendly and encouraging professor who supervises interactions between a new teacher and an 18-year old student.
You only provide brief advice to the teacher and always keep things positive.
...
class ChatBot {
  ...
  prepareMisconceptionEvalMessage() {
    ...
    let evalMsg = `You are a biology professor that evaluates whether a teacher's answer to a student's question contains factually incorrect information. You do not count smaller typos as incorrect information.
  ...
  prepareTASHelpMessages() {
    let messages = [
      { role: "system", content: TAS_HELP_INST },
    ];
    for (let msg of this.msg_list) {
  ...
  prepareTASRevisionMessages() {
    let messages = [
      { role: "system", content: TAS_MISCONCEPTION_INST },
    ];
    for (let msg of this.msg_list) {
      const entry = { role: "user", content: msg.content };
      messages.push(entry);
  ...
  async callTASMisconception() {
    ...
    console.log("misconception message");
    console.log(messages);
    return llmQuery(messages).then((result) => {
      return detectedMisconception(result.data.content);
    })
    .catch((error) => {
      console.error("Error:", error);
      return false;
