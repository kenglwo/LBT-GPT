...
export const sendChatToGPT = async (conversationHistory, text, fileBase64) => {
  ...
  const shouldGenerateImage = await askGPTShouldGenerateImage(
    text,
    gpt_4,
    headers,
    endpoint
  ...
  if (shouldGenerateImage) {
    ...
    const imageResponse = await generateImageWithDALLE(
      text,
      dalle2,
      headers,
      dalle_endpoint
...
const askGPTShouldGenerateImage = async (text, model, headers, endpoint) => {
  ...
  const messagesWithInquiry = [
    {
      role: 'system',
      content: `Is the user's input a request to generate an image? Please answer with yes or no.`
    },
    newUserMessage
  ]
