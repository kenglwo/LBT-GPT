...
// import ChatBot, { STUDENT_OPENING_SENTENCE, USER_OPENING_SENTENCE } from "../open_ai/chat_bot_v3";
import ChatBot, { STUDENT_OPENING_SENTENCE, USER_OPENING_SENTENCE } from "../open_ai/chat_bot_v6";
...
function counterUserMessages(messages) {
  return messages.reduce((count, obj) => {
    return count + (obj.isUser === true ? 1 : 0);
  }, 0);
}
...
const useChat = ({ scrollToBottom, setCompleted, setUsedBot, sInfo }) => {
  ...
  const chatLog = useRef({
    main: [{text: USER_OPENING_SENTENCE, isUser: true, date: time}],  // {text: STUDENT_OPENING_SENTENCE, isUser: false, date: time},
    help: [],
  ...
  const sendMessage = async (e) => {
    ...
    if (sendActive && (message.trim() !== "")) {  // add new user message to chain
      ...
      await new Promise((resolve) => setTimeout(resolve, 1000));
      setSend(true);
      if (counterUserMessages(chatLog.current.main) >= minMessages) {
        setCompleted(true);
      }
    }
  };
