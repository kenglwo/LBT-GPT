...
import { setCookie, getCookie, registerUser } from './components/misc/userManagement';
...
import { QATopicsHuman, QATopicsLLM } from './components/4-3-eukaryotic-cells/eukaryoticCellText_v2';
...
const App = () => {
  ...
  const sInfo = useRef({  // contains info we want to track during session
    uuid: userID,
  ...
  useEffect(() => {
    if (!hasRegistered) { // check global flag
      if (!sInfo.current.isMobile) {  // assign user to condition
        console.log("registering user");
        registerUser(sInfo).then(() => {
  ...
  useEffect(() => {  // Write this to sInfo on change.
  ...
  switch (screenState) {
    case 'intialization':
      if (!sInfo.current.isMobile) {  // avoid mobile devices
    ...
    case 'welcome':  // condition-specific instructions
      switch (sInfo.current.condition) {
        case 'llm-chatbot':
          currentScreen = <WelcomeScreen setState={setState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
          break;
        case 'teacher-qa':
          currentScreen = <WelcomeScreenQA setState={setState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
          break;
        case 'teacher-qa-bot':
          currentScreen = <WelcomeScreenQA setState={setState} sInfo={sInfo} returnSurvey={returnSurvey} />;
          break;
        case 'llm-qa-bot':
          ...
          break;
        case 'reading':
          currentScreen = <WelcomeScreenControl setState={setState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
          break;
        default:  // error state
          currentScreen = null;
          console.error('Encountered invalid condition', sInfo.current.condition);
          break;
      }
      break;
      ...
      switch (sInfo.current.condition) {
        case 'llm-chatbot':
          if (screenSubState === "read") {
            currentScreen = <ReadingScreen setSubState={setSubState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
          } else {
            currentScreen = <LBTApplication setState={setState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
          }
          break;
        case 'teacher-qa':
          currentScreen = <LBTApplicationQA setState={setState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
          break;
        case 'teacher-qa-bot':
          if (screenSubState === "read") {
            currentScreen = <ReadingScreen setSubState={setSubState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
          } else {
            currentScreen = <LBTApplicationQABot setState={setState} sInfo={sInfo} returnSurvey={returnSurvey} QATopics={QATopicsHuman} />;
          }
          break;
        case 'llm-qa-bot':
          if (screenSubState === "read") {
            ...
            currentScreen = <LBTApplicationQABot setState={setState} sInfo={sInfo} returnSurvey={returnSurvey} QATopics={QATopicsLLM} />;
          }
          break;
        case 'reading':
          currentScreen = <LBTApplicationControl setState={setState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
          break;
        default:  // error state
          currentScreen = null;
          console.error('Encountered invalid condition', sInfo.current.condition);
    ...
    case 'knowledge':  // post-test (same for all conditions)
      currentScreen = <KnowledgeScreen setState={setState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
      break;
    case 'survey':  // UX survey (chatbot or basic version)
      switch (sInfo.current.condition) {
        case 'llm-chatbot':
          currentScreen = <SurveyScreen setState={setState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
          break;
        case 'teacher-qa':
          currentScreen = <SurveyScreenQA setState={setState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
          break;
        case 'teacher-qa-bot':
          currentScreen = <SurveyScreenQA setState={setState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
          break;
        case 'llm-qa-bot':
          ...
          break;
        case 'reading':
          currentScreen = <SurveyScreenControl setState={setState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
          break;
        default:  // error state
          currentScreen = null;
          console.error('Encountered invalid condition', sInfo.current.condition);
    ...
    case 'demographics':  // demographic survey (same for all conditions)
      currentScreen = <DemographicScreen setState={setState} sInfo={sInfo} returnSurvey={returnSurvey}/>;
      break;
  ...
  return <div>
    {currentScreen}
    <Modal show={showModal} setShow={setShowModal} screenState={screenState}/>
    <ReturnModal show={showReturn} setShow={setShowReturn} terminateSurvey={terminateSurvey}/>
  </div>;
